...
public interface DeferrableInteraction<T extends DeferrableInteractionEvent> {
    ...
    default @NotNull Publisher<?> onError(@NotNull T event, @NotNull Throwable exception) {
        List<TopLevelMessageComponent> components = ResponseMessage.buildComponents(CommonResponseMessages.INTERACTION_CRASH, this, event, exception);

        return event.deferReply().onErrorComplete().then(
