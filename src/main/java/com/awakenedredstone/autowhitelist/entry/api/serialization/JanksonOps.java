...
package com.awakenedredstone.autowhitelist.entry.api.serialization;
...
import com.awakenedredstone.moondust.jankson.util.Numbers;
...
public class JanksonOps implements DynamicOps<JsonElement> {
    ...
    public <U> U convertTo(final DynamicOps<U> outOps, final JsonElement input) {
        ...
        try {
            ...
            return outOps.createLong(number);
    ...
    public DataResult<Number> getNumberValue(final JsonElement input) {
        if (input instanceof JsonPrimitive<?> primitive) {
            if (primitive.isNumber()) {
                return DataResult.success(primitive.asNumber(0));
            }
            if (compressed && primitive.isText()) {
                try {
    ...
    @Override
    public JsonElement createNumeric(final Number number) {
    ...
    public DataResult<String> getStringValue(final JsonElement input) {
        if (input instanceof JsonPrimitive<?> primitive) {
            if (primitive.isText() || primitive.isNumber() && compressed) {
    ...
    public DataResult<JsonElement> mergeToMap(final JsonElement map, final JsonElement key, final JsonElement value) {
        ...
        if (!(key instanceof JsonPrimitive<?> primitive) || !(primitive.isText()) && !compressed) {
    ...
    public DataResult<Stream<Pair<JsonElement, JsonElement>>> getMapValues(final JsonElement input) {
        ...
        return DataResult.success(jsonObject.entrySet().stream().map(entry -> Pair.of(new JsonText(entry.getKey()), entry.getValue() instanceof JsonNull ? null : entry.getValue())));
    }
    ...
    public DataResult<MapLike<JsonElement>> getMap(final JsonElement input) {
        ...
        return DataResult.success(new MapLike<>() {
            ...
            public Stream<Pair<JsonElement, JsonElement>> entries() {
                return object.entrySet().stream().map(entry -> Pair.of(new JsonText(entry.getKey()), entry.getValue()));
            }
    ...
    public DataResult<Stream<JsonElement>> getStream(final JsonElement input) {
        if (input instanceof JsonArray array) {
            return DataResult.success(array.stream().map(element -> element instanceof JsonNull ? null : element));
        }
